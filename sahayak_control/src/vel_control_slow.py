...
class sahayak:
    def __init__(self):
        ...
        for joint_indx in range(1, 5):
            ...
            self.joint_pub.update(
                {
                    "joint{}".format(joint_indx): rospy.Publisher(
                        "/sahayak/joint{}_vel_controller/command".format(joint_indx),
                        Float64,
                        queue_size=10,
                    )
                }
    ...
    def move(self):
        ...
        while not rospy.is_shutdown():
            with keyboard.Listener(
                on_press=bot.on_press, on_release=on_release
            ) as listener:
                listener.join()
    ...
    def on_press(self, key):
        try:
            if key == Key.up:
                ...
                self.joint_pub["joint1"].publish(SPEED * 2 * PI / 60)  # RPM
                self.joint_pub["joint2"].publish(-SPEED * 2 * PI / 60)  # RPM
                self.joint_pub["joint3"].publish(SPEED * 2 * PI / 60)  # RPM
                self.joint_pub["joint4"].publish(-SPEED * 2 * PI / 60)  # RPM
